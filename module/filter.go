...
func Filter(w http.ResponseWriter, r *http.Request) {
	...
	if cookieUser != "Visiteur" {
		...
		for rows.Next() {
			...
			err := rows.Scan(&post.Id, &post.Username, &post.Date, &post.Token, &post.Message, &post.Golang, &post.JavaScript, &post.Python, &post.Rust, &post.HTML_CSS, &post.Angular, &post.Autre, &post.Like, &post.Dislike)
			if err != nil {
				handle500(w, err)
				return
			}
